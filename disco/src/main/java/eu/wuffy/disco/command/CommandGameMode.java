...
public class CommandGameMode implements CommandExecutor, TabExecutor {
	...
	private static final List<String> GAMEMODE_LIST = ImmutableList.of("Disco", "Adventure", "Creative", "Spectator");
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0) {
			if (sender.hasPermission("wuffy.gamemode")) {
				if (player == null) {
					sender.sendMessage(Disco.PREFIX + "§7Die Console kann ihren §aGameMode §7nicht ändern§8.");
					return true;
				}
				...
				player.sendMessage(Disco.PREFIX + "§7Dein GameMode wurde zu "
						+ this.getGameModeName(player.getGameMode()) + " §7geändert§8.");
				return true;
			}
			sender.sendMessage(Disco.PREFIX + "§7Du hast keine §aRechte §7um diesen command zu nutzen§8.");
			return true;
		}
		...
		if (found.size() == 0) {
			sender.sendMessage(Disco.PREFIX + "§7Der angegebene GameMode §c\"" + args[0] + "\" §7konnte nicht gefunden werden§8.\n" +
								Disco.PREFIX + "§7/gm §8[§a0§7/§21§7/§52§7/§D3§8]§8.");
			return true;
		}
		...
		if (args.length == 1) {
			...
			if (sender.hasPermission("wuffy.gamemode")) {
				...
				player.sendMessage(Disco.PREFIX + "§7Dein GameMode wurde zu " + this.getGameModeName(player.getGameMode()) + " §7geändert§8.");
				return true;
			}
			sender.sendMessage(Disco.PREFIX + "§7Du hast keine §aRechte §7um diesen command zu nutzen§8.");
			return true;
		...
		if (target == null) {
			sender.sendMessage(Disco.PREFIX + "§7Der angegebene §aSpieler §7ist nicht online§8.");
			return true;
		}
		...
		if (player != null && target.equals(player))
			sender.sendMessage(Disco.PREFIX + "§7Dein GameMode wurde zu " + this.getGameModeName(player.getGameMode()) + " §7geändert§8.");
		else {
			target.sendMessage(Disco.PREFIX + "§7Dein GameMode wurde von " + (player == null ? "der §c" : "§a") + sender.getName() + " §7zu " + this.getGameModeName(gameMode) + " §7geändert§8.");
			sender.sendMessage(Disco.PREFIX + "§7Der GameMode von §a" + target.getName() + " §7wurde zu " + this.getGameModeName(gameMode) + " §7geändert§8.");
		}
		return true;
	}
