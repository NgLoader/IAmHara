package eu.wuffy.survival.command.tpa;
...
public class CommandTpaccept implements CommandExecutor, TabExecutor {

	private final TPAHandler tpaHandler;

	public CommandTpaccept(Survival survival) {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (!sender.hasPermission("survival.tpa.accept")) {
		...
		if (!requests.canRequest(target)) {
			if (!requests.acceptRequest(target)) {
